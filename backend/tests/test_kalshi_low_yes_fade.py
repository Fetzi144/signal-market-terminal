...
@pytest.mark.asyncio
async def test_kalshi_low_yes_fade_evaluator_matches_only_fixed_rule(session):
    ...
    evaluation = evaluate_kalshi_low_yes_fade_signal(signal, market_platform="kalshi")

    assert evaluation.in_scope is True
    assert evaluation.eligible is True
    assert evaluation.diagnostics["intended_direction"] == "buy_no"
    assert evaluation.diagnostics["edge_per_share"] == "0.050000"

    signal.price_at_fire = Decimal("0.250000")
    evaluation = evaluate_kalshi_low_yes_fade_signal(signal, market_platform="kalshi")

    assert evaluation.in_scope is False
    assert evaluation.eligible is False
    assert evaluation.reason_code == "not_kalshi_low_yes_fade_price_bucket"


@pytest.mark.asyncio
async def test_kalshi_low_yes_fade_registry_seeds_shadow_candidate(session):
...
async def test_kalshi_low_yes_fade_paper_lane_opens_buy_no_trade_under_candidate_run(session):
    ...
    result = await run_kalshi_low_yes_fade_paper_lane(
        session,
        [signal],
...
@pytest.mark.asyncio
async def test_kalshi_low_yes_fade_snapshot_normalizes_to_research_lane(session):
    now = datetime.now(timezone.utc).replace(microsecond=0)
    market = make_market(session, platform="kalshi", end_date=now + timedelta(days=1))
    outcome = make_outcome(session, market.id, name="Yes")
    make_signal(
    ...
    payload = kalshi_low_yes_fade_lane_payload(snapshot)
    ...
    assert payload["source_kind"] == "kalshi_low_yes_fade_snapshot"
