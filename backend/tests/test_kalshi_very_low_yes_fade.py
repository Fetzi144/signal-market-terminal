...
class _FakeKalshiConnector:
    ...
    async def fetch_orderbook(self, token_id: str) -> RawOrderbook:
        ...
        return RawOrderbook(
            token_id=token_id,
            bids=[["0.075000", "100000"]],
            asks=[["0.085000", "100000"]],
    ...
    async def fetch_midpoints(self, token_ids: list[str]) -> dict[str, Decimal]:
        ...
        return {token_id: Decimal("0.075000") for token_id in token_ids}
...
@pytest.mark.asyncio
async def test_kalshi_very_low_yes_fade_evaluator_matches_only_fixed_rule(session):
    ...
    signal = make_signal(
        ...
        price_at_fire=Decimal("0.075000"),
        expected_value=Decimal("-0.045000"),
        estimated_probability=Decimal("0.0300"),
    )

    evaluation = evaluate_kalshi_very_low_yes_fade_signal(signal, market_platform="kalshi")

    assert evaluation.in_scope is True
    assert evaluation.eligible is True
    assert evaluation.diagnostics["intended_direction"] == "buy_no"
    assert evaluation.diagnostics["edge_per_share"] == "0.045000"
    ...
    assert evaluation.reason_code == "not_kalshi_very_low_yes_fade_price_bucket"

    signal.price_at_fire = Decimal("0.075000")
    signal.timeframe = "1h"
    evaluation = evaluate_kalshi_very_low_yes_fade_signal(signal, market_platform="kalshi")

    assert evaluation.in_scope is False
    assert evaluation.eligible is False
    assert evaluation.reason_code == "kalshi_very_low_yes_fade_wrong_timeframe"


@pytest.mark.asyncio
async def test_kalshi_very_low_yes_fade_registry_seeds_shadow_candidate(session):
...
@pytest.mark.asyncio
async def test_kalshi_very_low_yes_fade_paper_lane_captures_fresh_book_and_opens_buy_no(
    session,
    monkeypatch,
):
    ...
    market = make_market(
        session,
        platform="kalshi",
        question="Will the test contract fade?",
        end_date=now + timedelta(days=1),
        last_liquidity=Decimal("5000.00"),
        active=True,
    )
    outcome = make_outcome(session, market.id, name="Yes", token_id="KTEST-VERYLOW:yes")
    signal = make_signal(
        session,
        market.id,
        outcome.id,
    ...
    result = await run_kalshi_very_low_yes_fade_paper_lane(
        session,
        [signal],
    ...
    assert fake_connector.orderbook_tokens == ["KTEST-VERYLOW:yes"]
    ...
    assert decision.details["market_price"] == "0.075000"
...
async def test_scheduler_runs_kalshi_lane_without_default_strategy_run(session, monkeypatch):
    ...
    market = make_market(
        session,
        platform="kalshi",
        question="Will scheduler test fade?",
        end_date=now + timedelta(days=1),
        last_liquidity=Decimal("5000.00"),
        active=True,
    )
    outcome = make_outcome(session, market.id, name="Yes", token_id="KTEST-VERYLOW-SCHED:yes")
    signal = make_signal(
        session,
        market.id,
        outcome.id,
...
@pytest.mark.asyncio
async def test_kalshi_very_low_yes_fade_snapshot_normalizes_to_research_lane(session):
    now = datetime.now(timezone.utc).replace(microsecond=0)
    market = make_market(session, platform="kalshi", end_date=now + timedelta(days=1))
    outcome = make_outcome(session, market.id, name="Yes")
    make_signal(
    ...
    payload = kalshi_very_low_yes_fade_lane_payload(snapshot)
    ...
    assert payload["source_kind"] == "kalshi_very_low_yes_fade_snapshot"
