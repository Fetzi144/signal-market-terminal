...
async def test_kalshi_cheap_yes_follow_registry_seeds_shadow_candidate(session):
    ...
    assert version.config_json["paper_quarantine"]["enabled"] is True


@pytest.mark.asyncio
async def test_kalshi_cheap_yes_follow_paper_lane_is_quarantined_after_negative_forward_paper(
    session,
    monkeypatch,
):
    ...
    assert result["targeted_orderbook_captures"] == 0
    assert result["opened_count"] == 0
    assert result["skip_counts"] == {"kalshi_cheap_yes_follow_forward_paper_quarantine": 1}
    assert fake_connector.orderbook_tokens == []
    assert fake_connector.closed is False
    ...
    assert decision.reason_code == "kalshi_cheap_yes_follow_forward_paper_quarantine"
    assert signal.details["kalshi_cheap_yes_follow"]["paper_quarantined"] is True
...
async def test_scheduler_runs_kalshi_lane_without_default_strategy_run(session, monkeypatch):
    ...
    assert trades == []
    decision = (await session.execute(select(ExecutionDecision))).scalars().one()
    assert decision.strategy_run_id == strategy_run.id
    assert decision.reason_code == "kalshi_cheap_yes_follow_forward_paper_quarantine"
