...
STRATEGY_FAMILY = "kalshi_low_yes_fade"
STRATEGY_NAME = "kalshi_low_yes_fade_v1"
STRATEGY_VERSION_KEY = "kalshi_low_yes_fade_v1"
SIGNAL_DETAILS_KEY = "kalshi_low_yes_fade"

MIN_YES_PRICE = Decimal("0.10")
MAX_YES_PRICE = Decimal("0.20")
ZERO = Decimal("0")


@dataclass(frozen=True)
class KalshiLowYesFadeEvaluation:
    in_scope: bool
    eligible: bool
    reason_code: str
    reason_label: str
...
def _reason_label(reason_code: str) -> str:
    labels = {
        "kalshi_low_yes_fade_candidate": "Kalshi low-YES fade candidate",
        "kalshi_low_yes_fade_missing_direction": "Missing price-move direction",
        "kalshi_low_yes_fade_missing_outcome": "Missing outcome",
        "kalshi_low_yes_fade_missing_price": "Missing YES price",
        "kalshi_low_yes_fade_missing_probability": "Missing estimated probability",
...
def evaluate_kalshi_low_yes_fade_signal(
    signal: Signal,
    *,
    market_platform: str | None = None,
) -> KalshiLowYesFadeEvaluation:
    """Evaluate the fixed Kalshi low-YES fade rule without mutating state."""
    ...
    if signal_type != "price_move" or platform != "kalshi":
        return KalshiLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="not_kalshi_low_yes_fade_universe",
            reason_label="Not in Kalshi low-YES fade universe",
    ...
    if not direction:
        return KalshiLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_low_yes_fade_missing_direction",
            reason_label=_reason_label("kalshi_low_yes_fade_missing_direction"),
            diagnostics=diagnostics,
        )
    if direction != "down":
        return KalshiLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="not_kalshi_low_yes_fade_direction",
    ...
    if signal.outcome_id is None:
        return KalshiLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_low_yes_fade_missing_outcome",
            reason_label=_reason_label("kalshi_low_yes_fade_missing_outcome"),
            diagnostics=diagnostics,
        )
    if yes_price is None:
        return KalshiLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_low_yes_fade_missing_price",
            reason_label=_reason_label("kalshi_low_yes_fade_missing_price"),
            diagnostics=diagnostics,
        )
    if yes_price < MIN_YES_PRICE or yes_price >= MAX_YES_PRICE:
        return KalshiLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="not_kalshi_low_yes_fade_price_bucket",
            reason_label="YES price outside low fade bucket",
            diagnostics=diagnostics,
        )
    if expected_value is None:
        return KalshiLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_low_yes_fade_missing_expected_value",
            reason_label=_reason_label("kalshi_low_yes_fade_missing_expected_value"),
            diagnostics=diagnostics,
        )
    if expected_value >= ZERO:
        return KalshiLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="kalshi_low_yes_fade_expected_value_not_negative",
            reason_label=_reason_label("kalshi_low_yes_fade_expected_value_not_negative"),
            diagnostics=diagnostics,
        )
    if estimated_probability is None:
        return KalshiLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_low_yes_fade_missing_probability",
            reason_label=_reason_label("kalshi_low_yes_fade_missing_probability"),
            diagnostics=diagnostics,
        )
    if estimated_probability >= yes_price:
        return KalshiLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_low_yes_fade_probability_not_below_price",
            reason_label=_reason_label("kalshi_low_yes_fade_probability_not_below_price"),
            diagnostics=diagnostics,
        )
    ...
    return KalshiLowYesFadeEvaluation(
        in_scope=True,
        eligible=True,
        reason_code="kalshi_low_yes_fade_candidate",
        reason_label=_reason_label("kalshi_low_yes_fade_candidate"),
        diagnostics=diagnostics,
    )


async def ensure_active_kalshi_low_yes_fade_run(
    ...
    *,
    ...
    strategy_run = StrategyRun(
        ...
        contract_snapshot={
            ...
            "strategy_version_label": version.version_label if version is not None else "Kalshi Low-YES Fade v1",
...
async def load_unprocessed_kalshi_low_yes_fade_signals(
    session: AsyncSession,
    strategy_run: StrategyRun,
    *,
    ...
    filtered = [
        ...
        if evaluate_kalshi_low_yes_fade_signal(
            signal,
...
async def run_kalshi_low_yes_fade_paper_lane(
    session: AsyncSession,
    signals: Sequence[Signal],
    *,
    ...
    backlog_signals = await load_unprocessed_kalshi_low_yes_fade_signals(
        session,
        strategy_run,
    ...
    for signal, attempt_kind in work_items:
        evaluation = evaluate_kalshi_low_yes_fade_signal(
            signal,
            market_platform=platforms.get(signal.market_id),
        )
    ...
    if candidate_count or run_created or expired_pending or finalized_orderbook_context:
        logger.info(
            "Kalshi low-YES fade paper lane: opened %d trade(s) from %d candidate signal(s)",
            opened_count,
            candidate_count,
        )
        if skip_counts:
