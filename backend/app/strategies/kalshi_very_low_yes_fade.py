...
STRATEGY_FAMILY = "kalshi_very_low_yes_fade"
STRATEGY_NAME = "kalshi_very_low_yes_fade_v1"
STRATEGY_VERSION_KEY = "kalshi_very_low_yes_fade_v1"
SIGNAL_DETAILS_KEY = "kalshi_very_low_yes_fade"

MIN_YES_PRICE = Decimal("0.05")
MAX_YES_PRICE = Decimal("0.10")
TIMEFRAME = "30m"
ZERO = Decimal("0")


@dataclass(frozen=True)
class KalshiVeryLowYesFadeEvaluation:
    in_scope: bool
    eligible: bool
    reason_code: str
    reason_label: str
...
def _reason_label(reason_code: str) -> str:
    labels = {
        "kalshi_very_low_yes_fade_candidate": "Kalshi very-low-YES fade candidate",
        "kalshi_very_low_yes_fade_wrong_timeframe": "Not a 30m price-move signal",
        "kalshi_very_low_yes_fade_missing_direction": "Missing price-move direction",
        "kalshi_very_low_yes_fade_missing_outcome": "Missing outcome",
        "kalshi_very_low_yes_fade_missing_price": "Missing YES price",
        "kalshi_very_low_yes_fade_missing_probability": "Missing estimated probability",
        "kalshi_very_low_yes_fade_missing_expected_value": "Missing expected value",
        "kalshi_very_low_yes_fade_expected_value_not_negative": "Expected value is not negative",
        "kalshi_very_low_yes_fade_probability_not_below_price": "Probability is not below YES price",
        "kalshi_very_low_yes_fade_current_price_unavailable": "Current YES price unavailable",
        "kalshi_very_low_yes_fade_current_price_outside_bucket": "Current YES price outside very-low fade bucket",
        "kalshi_very_low_yes_fade_current_probability_not_below_price": "Current probability is not below YES price",
    }
    return labels.get(reason_code, reason_code.replace("_", " "))


def evaluate_kalshi_very_low_yes_fade_signal(
    signal: Signal,
    *,
    market_platform: str | None = None,
) -> KalshiVeryLowYesFadeEvaluation:
    """Evaluate the fixed Kalshi very-low-YES fade rule without mutating state."""

    details = _details(signal)
    signal_type = _normalize_text(signal.signal_type)
    timeframe = _normalize_text(signal.timeframe)
    ...
    diagnostics = {
        ...
        "timeframe_required": TIMEFRAME,
        "intended_direction": "buy_no",
    }

    if signal_type != "price_move" or platform != "kalshi":
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="not_kalshi_very_low_yes_fade_universe",
            reason_label="Not in Kalshi very-low-YES fade universe",
    ...
    if timeframe != TIMEFRAME:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_wrong_timeframe",
            reason_label=_reason_label("kalshi_very_low_yes_fade_wrong_timeframe"),
            diagnostics=diagnostics,
        )
    if not direction:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_missing_direction",
            reason_label=_reason_label("kalshi_very_low_yes_fade_missing_direction"),
            diagnostics=diagnostics,
        )
    if direction != "down":
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="not_kalshi_very_low_yes_fade_direction",
    ...
    if signal.outcome_id is None:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_missing_outcome",
            reason_label=_reason_label("kalshi_very_low_yes_fade_missing_outcome"),
            diagnostics=diagnostics,
        )
    if yes_price is None:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_missing_price",
            reason_label=_reason_label("kalshi_very_low_yes_fade_missing_price"),
            diagnostics=diagnostics,
        )
    if yes_price < MIN_YES_PRICE or yes_price >= MAX_YES_PRICE:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="not_kalshi_very_low_yes_fade_price_bucket",
            reason_label="YES price outside very-low fade bucket",
            diagnostics=diagnostics,
        )
    if expected_value is None:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_missing_expected_value",
            reason_label=_reason_label("kalshi_very_low_yes_fade_missing_expected_value"),
            diagnostics=diagnostics,
        )
    if expected_value >= ZERO:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=False,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_expected_value_not_negative",
            reason_label=_reason_label("kalshi_very_low_yes_fade_expected_value_not_negative"),
            diagnostics=diagnostics,
        )
    if estimated_probability is None:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_missing_probability",
            reason_label=_reason_label("kalshi_very_low_yes_fade_missing_probability"),
            diagnostics=diagnostics,
        )
    if estimated_probability >= yes_price:
        return KalshiVeryLowYesFadeEvaluation(
            in_scope=True,
            eligible=False,
            reason_code="kalshi_very_low_yes_fade_probability_not_below_price",
            reason_label=_reason_label("kalshi_very_low_yes_fade_probability_not_below_price"),
            diagnostics=diagnostics,
        )

    no_entry_price = (Decimal("1") - yes_price).quantize(Decimal("0.000001"))
    diagnostics["no_entry_price"] = str(no_entry_price)
    diagnostics["edge_per_share"] = str((yes_price - estimated_probability).quantize(Decimal("0.000001")))
    return KalshiVeryLowYesFadeEvaluation(
        in_scope=True,
        eligible=True,
        reason_code="kalshi_very_low_yes_fade_candidate",
        reason_label=_reason_label("kalshi_very_low_yes_fade_candidate"),
        diagnostics=diagnostics,
    )


async def ensure_active_kalshi_very_low_yes_fade_run(
    ...
    *,
    ...
    strategy_run = StrategyRun(
        ...
        contract_snapshot={
            ...
            "strategy_version_label": version.version_label if version is not None else "Kalshi Very-Low-YES Fade v1",
...
async def load_unprocessed_kalshi_very_low_yes_fade_signals(
    session: AsyncSession,
    strategy_run: StrategyRun,
    *,
    ...
    filtered = [
        ...
        if evaluate_kalshi_very_low_yes_fade_signal(
            signal,
...
async def run_kalshi_very_low_yes_fade_paper_lane(
    session: AsyncSession,
    signals: Sequence[Signal],
    *,
    ...
    strategy_run, run_created = await ensure_active_kalshi_very_low_yes_fade_run(session)
    ...
    backlog_signals = await load_unprocessed_kalshi_very_low_yes_fade_signals(
        session,
        strategy_run,
    ...
    for signal, attempt_kind in work_items:
        evaluation = evaluate_kalshi_very_low_yes_fade_signal(
            signal,
            market_platform=platforms.get(signal.market_id),
        )
        ...
        if (
            ...
            targeted_capture_result = await capture_targeted_kalshi_orderbook_snapshot(
                ...
                log_context="Kalshi very-low-YES fade",
            )
            changed = True
            if targeted_capture_result.get("captured"):
                ...
                if current_midpoint is None:
                    current_precheck_reason_code = "kalshi_very_low_yes_fade_current_price_unavailable"
                    ...
                    current_precheck_reason_code = "kalshi_very_low_yes_fade_current_price_outside_bucket"
                    current_precheck_reason_label = _reason_label(current_precheck_reason_code)
                elif signal.estimated_probability is None or _decimal(signal.estimated_probability) is None:
                    current_precheck_reason_code = "kalshi_very_low_yes_fade_missing_probability"
                    current_precheck_reason_label = _reason_label(current_precheck_reason_code)
                elif (_decimal(signal.estimated_probability) or ZERO) >= current_midpoint:
                    current_precheck_reason_code = "kalshi_very_low_yes_fade_current_probability_not_below_price"
                    current_precheck_reason_label = _reason_label(current_precheck_reason_code)
                else:
    ...
    if candidate_count or run_created or expired_pending or finalized_orderbook_context:
        logger.info(
            "Kalshi very-low-YES fade paper lane: opened %d trade(s) from %d candidate signal(s)",
            opened_count,
            candidate_count,
        )
        if targeted_orderbook_captures or targeted_orderbook_skips:
            logger.info(
                "Kalshi very-low-YES fade targeted orderbooks: captured=%d skips=%s",
                targeted_orderbook_captures,
                targeted_orderbook_skips,
            )
        if skip_counts:
            logger.info("Kalshi very-low-YES fade skips by reason: %s", skip_counts)
