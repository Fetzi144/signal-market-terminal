...
from app.strategies.kalshi_low_yes_fade import (
    MAX_YES_PRICE,
    MIN_YES_PRICE,
    STRATEGY_FAMILY,
    STRATEGY_NAME,
    STRATEGY_VERSION_KEY,
    ensure_active_kalshi_low_yes_fade_run,
    evaluate_kalshi_low_yes_fade_signal,
    run_kalshi_low_yes_fade_paper_lane,
)
from app.strategy_runs.service import get_active_strategy_run

ARTIFACT_DIR = "docs/research-lab/kalshi-low-yes-fade"
SCHEMA_VERSION = "kalshi_low_yes_fade_snapshot_v1"
...
def _artifact_stem(as_of: datetime, window_days: int) -> str:
    return f"{as_of.date().isoformat()}-kalshi-low-yes-fade-{window_days}d"
...
async def _load_matching_signals(
    ...
    *,
    ...
    for signal, platform in rows:
        if evaluate_kalshi_low_yes_fade_signal(signal, market_platform=platform).eligible:
...
def _profitability_verdict(
    *,
    ...
    if strategy_run and strategy_run.get("started_at"):
        ...
        actions.append(
            {
                "step": "start_kalshi_low_yes_fade_paper_run",
                "priority_score": 95,
    ...
    if int(historical.get("matching_signals") or 0) == 0:
        blockers.append("no_matching_kalshi_low_yes_fade_signals")
        ...
        actions.append(
            {
                ...
                "why_ev": "This is the only candidate that survived the alpha gauntlet, but it still needs execution-adjusted forward evidence.",
...
async def build_kalshi_low_yes_fade_snapshot(
    session: AsyncSession,
    *,
    window_days: int = 30,
...
def kalshi_low_yes_fade_lane_payload(snapshot: dict[str, Any]) -> dict[str, Any]:
    paper = snapshot.get("paper") or {}
    return {
        "family": STRATEGY_FAMILY,
        "strategy_version": STRATEGY_VERSION_KEY,
        "lane": "paper_forward_gate",
        "source_kind": "kalshi_low_yes_fade_snapshot",
...
Kalshi `price_move`, direction `down`, YES price `0.10 <= p < 0.20`, expected value `< 0`, expressed as `buy_no`.
...
async def generate_kalshi_low_yes_fade_artifact(
    session: AsyncSession,
    *,
    window_days: int = 30,
    max_signals: int = 5000,
    ...
    if seed_paper:
        ...
        await ensure_active_kalshi_low_yes_fade_run(session, started_at=started_at)
        seed_result = await run_kalshi_low_yes_fade_paper_lane(
            session,
            signals,
    ...
    return {
        "snapshot": _json_safe(snapshot),
        "lane_payload": _json_safe(kalshi_low_yes_fade_lane_payload(snapshot)),
