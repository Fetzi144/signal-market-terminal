...
from app.strategies.kalshi_very_low_yes_fade import (
    MAX_YES_PRICE,
    MIN_YES_PRICE,
    STRATEGY_FAMILY,
    STRATEGY_NAME,
    ...
    TIMEFRAME,
    ensure_active_kalshi_very_low_yes_fade_run,
    evaluate_kalshi_very_low_yes_fade_signal,
    run_kalshi_very_low_yes_fade_paper_lane,
)
from app.strategy_runs.service import get_active_strategy_run

ARTIFACT_DIR = "docs/research-lab/kalshi-very-low-yes-fade"
SCHEMA_VERSION = "kalshi_very_low_yes_fade_snapshot_v1"
...
def _artifact_stem(as_of: datetime, window_days: int) -> str:
    return f"{as_of.date().isoformat()}-kalshi-very-low-yes-fade-{window_days}d"
...
async def _load_matching_signals(
    ...
    *,
    ...
    broad_limit = max(1, min(int(max_signals), 50_000))
    rows = (
    ...
    for signal, platform in rows:
        if evaluate_kalshi_very_low_yes_fade_signal(signal, market_platform=platform).eligible:
...
def _profitability_verdict(
    *,
    ...
    if strategy_run and strategy_run.get("started_at"):
        ...
        actions.append(
            {
                "step": "start_kalshi_very_low_yes_fade_paper_run",
                "priority_score": 95,
                "why_ev": "The historical alpha cannot compound evidence until the candidate lane is collecting forward paper decisions.",
                "operator_action": "Run the scheduler or seed the paper lane once, then monitor resolved trades.",
                "evidence": {"historical_matching_signals": historical.get("matching_signals")},
            }
        )

    if int(historical.get("matching_signals") or 0) == 0:
        blockers.append("no_matching_kalshi_very_low_yes_fade_signals")
    ...
    if not actions:
        actions.append(
            {
                "step": "continue_forward_paper_validation",
                "priority_score": 80 if verdict in {"watch", "research_ready"} else 60,
                "why_ev": "This Alpha Factory candidate survived the chronological holdout, but it still needs execution-adjusted forward evidence.",
...
async def build_kalshi_very_low_yes_fade_snapshot(
    session: AsyncSession,
    *,
    window_days: int = 30,
...
def kalshi_very_low_yes_fade_lane_payload(snapshot: dict[str, Any]) -> dict[str, Any]:
    paper = snapshot.get("paper") or {}
    return {
        "family": STRATEGY_FAMILY,
        "strategy_version": STRATEGY_VERSION_KEY,
        "lane": "paper_forward_gate",
        "source_kind": "kalshi_very_low_yes_fade_snapshot",
...
def _render_markdown(snapshot: dict[str, Any]) -> str:
    ...
    return f"""# Kalshi Very-Low-YES Fade
...
Kalshi `price_move`, timeframe `30m`, direction `down`, YES price `0.05 <= p < 0.10`, expected value `< 0`, expressed as `buy_no`.
...
async def generate_kalshi_very_low_yes_fade_artifact(
    session: AsyncSession,
    *,
    window_days: int = 30,
    max_signals: int = 5000,
    ...
    if seed_paper:
        ...
        await ensure_active_kalshi_very_low_yes_fade_run(session, started_at=started_at)
        seed_result = await run_kalshi_very_low_yes_fade_paper_lane(
            session,
            signals,
    ...
    return {
        "snapshot": _json_safe(snapshot),
        "lane_payload": _json_safe(kalshi_very_low_yes_fade_lane_payload(snapshot)),
