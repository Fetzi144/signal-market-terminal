...
from app.strategies.kalshi_cheap_yes_follow import (
    ...
    ensure_active_kalshi_cheap_yes_follow_run,
...
ARTIFACT_DIR = "docs/research-lab/kalshi-cheap-yes-follow"
SCHEMA_VERSION = "kalshi_cheap_yes_follow_snapshot_v1"
...
def _artifact_stem(as_of: datetime, window_days: int) -> str:
    return f"{as_of.date().isoformat()}-kalshi-cheap-yes-follow-{window_days}d"
...
async def _load_matching_signals(
    ...
    *,
    ...
    for signal, platform in rows:
        if evaluate_kalshi_cheap_yes_follow_signal(signal, market_platform=platform).eligible:
...
def _profitability_verdict(
    *,
    ...
    if strategy_run and strategy_run.get("started_at"):
        ...
        actions.append(
            {
                "step": "start_kalshi_cheap_yes_follow_paper_run",
                "priority_score": 95,
    ...
    if int(historical.get("matching_signals") or 0) == 0:
        blockers.append("no_matching_kalshi_cheap_yes_follow_signals")
        ...
        actions.append(
            {
                ...
                "why_ev": "The historical candidate still needs execution-adjusted forward evidence before it can be trusted.",
...
async def build_kalshi_cheap_yes_follow_snapshot(
    session: AsyncSession,
    *,
    window_days: int = 30,
    ...
    return {
        ...
        "rule": {
            ...
            "paper_min_ev_threshold": Decimal("0"),
        },
        "historical": historical,
        "paper": paper,
        "verdict": verdict,
...
def kalshi_cheap_yes_follow_lane_payload(snapshot: dict[str, Any]) -> dict[str, Any]:
    paper = snapshot.get("paper") or {}
    return {
        "family": STRATEGY_FAMILY,
        "strategy_version": STRATEGY_VERSION_KEY,
        "lane": "paper_forward_gate",
        "source_kind": "kalshi_cheap_yes_follow_snapshot",
...
def _render_markdown(snapshot: dict[str, Any]) -> str:
    ...
    return f"""# Kalshi Cheap-YES Follow
...
Kalshi `price_move`, direction `down`, YES price `0 < p < 0.05`, expected value `>= 0 and < 0.01`, expressed as `buy_yes`.
...
async def generate_kalshi_cheap_yes_follow_artifact(
    session: AsyncSession,
    *,
    window_days: int = 30,
    max_signals: int = 5000,
    ...
    if seed_paper:
        ...
        await ensure_active_kalshi_cheap_yes_follow_run(session, started_at=started_at)
        seed_result = await run_kalshi_cheap_yes_follow_paper_lane(
            session,
            signals,
    ...
    return {
        "snapshot": _json_safe(snapshot),
        "lane_payload": _json_safe(kalshi_cheap_yes_follow_lane_payload(snapshot)),
