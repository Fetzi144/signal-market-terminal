...
from app.alpha_rule_specs import ALPHA_KALSHI_4237F81367_V1
from app.models.execution_decision import ExecutionDecision
from app.models.market import Market
from app.models.paper_trade import PaperTrade
from app.models.signal import Signal
from app.strategies.alpha_rule_paper_lane import evaluate_alpha_rule_signal
from app.strategy_runs.service import get_active_strategy_run

SCHEMA_VERSION = "alpha_rule_paper_lane_snapshot_v1"
...
async def _paper_metrics(session: AsyncSession, *, blueprint: dict[str, Any]) -> dict[str, Any]:
    strategy_run = await get_active_strategy_run(session, str(blueprint["strategy_name"]))
...
def _profitability_verdict(
    *,
    ...
    if strategy_run and strategy_run.get("started_at"):
        ...
        actions.append(
            {
                "step": "start_alpha_rule_paper_run",
                "priority_score": 95,
                "why_ev": "The frozen Alpha Factory candidate cannot collect forward evidence until its paper run is active.",
                "operator_action": (
                    f"Run the scheduler and keep {blueprint.get('strategy_version')} paper-only "
                    "until the forward sample matures."
                ),
    ...
    if int(historical.get("matching_signals") or 0) == 0:
        blockers.append("no_matching_alpha_rule_signals")
        ...
        actions.append(
            {
                ...
                "why_ev": "The lane has historical signal-level evidence but still needs execution-adjusted forward proof.",
...
async def build_alpha_rule_paper_lane_snapshot(
    session: AsyncSession,
    *,
    ...
    paper = await _paper_metrics(session, blueprint=blueprint)
...
def alpha_rule_paper_lane_payload(snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "source_kind": "alpha_rule_paper_lane_snapshot",
